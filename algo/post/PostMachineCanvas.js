...
var PostMachineCanvas = function(canvas_id) {
...
PostMachineCanvas.prototype.setMachine = function(machine) {
	this.machine = machine;
	this.draw();
};

PostMachineCanvas.prototype.draw = function() {
...
PostMachineCanvas.prototype.drawEmptyCell = function(x, y, size, opacity) {
	var ctx = this.ctx;	
...
PostMachineCanvas.prototype.drawStripCell = function(x, y, value) {
	...
	var text = value ? '1' : '0';
	var text_size = ctx.measureText(text);	
	ctx.fillText(text, x + (cell_size - text_size.width) / 2 , y  +  (cell_size - this.font_size) / 2 );
}

PostMachineCanvas.prototype.drawMachine = function(x, y) {
	var ctx = this.ctx;
	...
	ctx.textAlign = 'left';
	ctx.textBaseline = 'top';
	var text = this.machine.rule_index;
	var text_size = ctx.measureText(text);	
